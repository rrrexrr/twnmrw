...
| 留言板 | ✅ | 写一张小便签贴到墙上，可以署名、选颜色、送爱心 |
| 邮件提醒 | ✅（可选） | 点了菜 / 完成了里程碑 / 有新留言，自动发邮件 |
...
**改完 css / js 之后**，把 `index.html` 里所有的 `?v=16` 换成新数字（VS Code：`Cmd+Shift+H` 全局替换，比如换成 `?v=17`），
...
开启后：**提交点单**、**完成一个里程碑**、**贴一张新留言**，都会自动发一封邮件（同一个里程碑来回点，1 小时内只发一次；给留言点爱心不发）。
需要先开好云端模式。
...
1. **Supabase**：SQL Editor → New query → 粘贴最新的 `supabase/notify.sql` → **什么都不用改**，直接 Run（会提示「网址没改，沿用之前填过的」，口令也不变）
2. **Apps Script**：把最新的 `supabase/apps-script.gs` 整段替换进去 → `Cmd+S` 保存 →
   **部署 → 管理部署 → 右边铅笔（编辑）→ 版本选「新版本」→ 部署**。网址不变，脚本属性也不用动。
   （只保存不发新版本的话，网站那边用的还是旧代码，新功能的邮件不会发）
...
- 在上面的便签里写字，选个颜色、选个署名（Rex / 洛洛 / 不署名，会记住这台设备上次选的），点 **贴上去**（电脑上 `Cmd+Enter` 也行）
- 没写完就切走也没关系，草稿会留着
- 墙上最新的在最前面；点 ♡ 送爱心，可以连点；右边 ⋯ 可以复制文字或删除
- 开了邮件提醒的话，每贴一张都会发邮件
